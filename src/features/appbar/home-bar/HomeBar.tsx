...
const HomeBar = () => {
  const { name } = useAppSelector((store) => store.auth);
  ...
  return (
    <div className="flex h-16 w-full items-center bg-green-700 px-2 text-neutral-50 shadow-lg md:px-6">
      ...
      <Link to="/admin/products">
        <button className="mr-2 rounded-md bg-amber-500 py-1 px-2 text-xl uppercase text-white hover:bg-amber-600 md:mr-4">Admin</button>
      </Link>
      {name ? (
        ...
        <Link to="/login">
          <button className="rounded-md bg-amber-500 py-1 px-2 text-xl uppercase text-white hover:bg-amber-600">Login</button>
        </Link>
      )}
    </div>
  );
